...
function testCompositeCreated() {
    ...
    if ! assertCurl 200 "curl https://$HOST:$PORT/product-composite/$PROD_ID_REVS_RECS -s --insecure"
    then
        echo -n "FAIL"
        return 1
    fi
...
function recreateComposite() {
    ...
    assertCurl 200 "curl -X DELETE https://$HOST:$PORT/product-composite/${productId} -s --insecure"
    curl --insecure -X POST https://$HOST:$PORT/product-composite -H "Content-Type: application/json" --data "$composite"
}
...
waitForService curl --insecure https://$HOST:$PORT/actuator/health
...
assertCurl 200 "curl https://$HOST:$PORT/product-composite/$PROD_ID_REVS_RECS -s --insecure"
assertEqual "$PROD_ID_REVS_RECS" $(echo $RESPONSE | jq .productId)
assertEqual 3 $(echo $RESPONSE | jq ".recommendations | length")
assertEqual 3 $(echo $RESPONSE | jq ".reviews | length")

# Verify that a 404 (Not Found) error is returned for a non existing productId ($PROD_ID_NOT_FOUND)
assertCurl 404 "curl https://$HOST:$PORT/product-composite/$PROD_ID_NOT_FOUND -s --insecure"

# Verify that no recommendations are returned for productId $PROD_ID_NO_RECS
assertCurl 200 "curl https://$HOST:$PORT/product-composite/$PROD_ID_NO_RECS -s --insecure"
...
assertCurl 200 "curl https://$HOST:$PORT/product-composite/$PROD_ID_NO_REVS -s --insecure"
assertEqual $PROD_ID_NO_REVS $(echo $RESPONSE | jq .productId)
assertEqual 3 $(echo $RESPONSE | jq ".recommendations | length")
assertEqual 0 $(echo $RESPONSE | jq ".reviews | length")

# Verify that a 422 (Unprocessable Entity) error is returned for a productId that is out of range (-1)
assertCurl 422 "curl https://$HOST:$PORT/product-composite/-1 -s --insecure"
assertEqual "\"Invalid productId: -1\"" "$(echo $RESPONSE | jq .message)"

# Verify that a 400 (Bad Request) error error is returned for a productId that is not a number, i.e. invalid format
assertCurl 400 "curl https://$HOST:$PORT/product-composite/invalidProductId -s --insecure"
